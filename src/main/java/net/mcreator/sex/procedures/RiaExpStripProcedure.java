...
public class RiaExpStripProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		SexMod.LOGGER.info(entity);
		...
		if (entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(new ItemStack(Items.GOLD_INGOT)) : false) {
			...
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal("<Ria> Hm, if you say so~"), false);
			if (((Entity) world.getEntitiesOfClass(PLayreEntity.class, AABB.ofSize(new Vec3(x, y, z), 8, 8, 8), e -> true).stream().sorted(new Object() {
				...
				}.compareDistOf(x, y, z)).findFirst().orElse(null))).setAnimation("animation.jenny.payment");
			}
			SexMod.queueServerWork(186, () -> {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.item.pickup")), SoundSource.VOICE, 1, 1);
					} else {
				...
				if (((Entity) world.getEntitiesOfClass(PLayreEntity.class, AABB.ofSize(new Vec3(x, y, z), 8, 8, 8), e -> true).stream().sorted(new Object() {
					...
					}.compareDistOf(x, y, z)).findFirst().orElse(null))).setAnimation("animation.ria.stripExp");
				}
				SexMod.queueServerWork(100, () -> {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = SexModEntities.RIASTRIPPEDEXPAN.get().spawn(_level, BlockPos.containing(entity.getX(), entity.getY(), entity.getZ()), MobSpawnType.MOB_SUMMONED);
						if (entityToSpawn != null) {
							entityToSpawn.setDeltaMovement(0, 0, 0);
						}
					}
			...
			if (!world.isClientSide() && world.getServer() != null)
				world.getServer().getPlayerList().broadcastSystemMessage(Component.literal("<Ria> I'll strip for a bar of gold~"), false);
		}
	}
}
